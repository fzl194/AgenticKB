...
mcp_key_open_kbs 开放库勾选。规则：
  - 幂等：旧行 key_hash 已存在于 mcp_keys → skip（跑两遍不产生第二把）；
    INSERT 亦带 ON CONFLICT DO NOTHING 兜底（并发双跑安全，唯一索引命中即吞）。
...
def main() -> int:
    ...
    try:
        ...
        for (user_id, key_hash, key_prefix, status, open_tools,
            ...
            domain_src = "open-kbs"
            if domain is None:
                domain = args.fallback_domain or first_binding.get(user_id)
                domain_src = "fallback" if args.fallback_domain else "user_domains"
            if domain is None and site_role == "admin":
                domain = get_default_domain()
                domain_src = "admin-default"
            ...
            if dropped:
                ...
                items = ", ".join(f"({n}@{d})" for n, d in dropped)
                dropped_detail.append(f"{username} -> [{items}]")

            # 4) 写库（每用户一个事务：钥匙行 + 开放库一起成败）
            if not args.dry_run:
                try:
                    with conn.cursor() as cur:
                        inserted = False
