...
--    上传状态机：(kb_id, actor, idempotency_key) 唯一保证客户端重试安全。
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS asset_upload_sessions (
    id                 TEXT PRIMARY KEY,
    kb_id              TEXT NOT NULL,                            -- M1 补 FK -> knowledge_bases(id)
    folder_id          TEXT,                                    -- M1 补 FK -> kb_folders(id)；NULL = 根目录
    actor              TEXT NOT NULL,
...
--    统一引用索引：加速 GC/审计。owner 表须另持真实 FK；本表不承担完整性（§8.5 末段）。
-- -----------------------------------------------------------------------------
...
COMMENT ON TABLE asset_storage_object_refs IS 'SRS §8.5: GC/审计加速冗余索引；多态 owner 不替代 owner 表真实 FK。';


-- -----------------------------------------------------------------------------
-- D. 新表：asset_file_audit_events（SRS §8.5）
--    append-only 文件管理审计流。不是可回滚版本表。
-- -----------------------------------------------------------------------------
...
COMMENT ON TABLE asset_file_audit_events IS 'SRS §8.5: append-only 审计流；before/after 仅记录差异快照，非版本表。';


-- -----------------------------------------------------------------------------
-- E. 新表：asset_storage_quotas（SRS §8.5 命名 kb_storage_quotas；按 D-003 落 asset_core）
--    KB 级存储配额：limit/reserved/used + version 乐观并发。
-- -----------------------------------------------------------------------------
...
COMMENT ON TABLE asset_storage_quotas IS 'SRS §8.5: KB 存储配额；reserved 供上传会话预留，commit 转 used，abort 释放；version 支持并发对账。';


-- -----------------------------------------------------------------------------
-- F. 新表：asset_storage_operations（SRS §8.5 outbox）
--    DB commit 后驱动的存储副作用：promote/cleanup/GC/notify。幂等可重试。
-- -----------------------------------------------------------------------------
...
COMMENT ON TABLE asset_storage_operations IS 'SRS §8.5: 存储副作用 outbox；幂等执行，可观测重试（attempts/next_retry_at）。';
