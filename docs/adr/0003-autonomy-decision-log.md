...
## D-041 ｜ 切片 v2.1：单行样板章节兜底吸收 + 小片治理顺序修复（2026-08-24）
- **背景**：D-038 后用户上传 GWFD 特性文档系列（6 篇，kb d5827c8b…）再验收，仍反馈"大量切片文字就几个字"。全库快照体检（只读）：v2 机制生效（表格全 whole 原子、角色全标注、p50 数百字），但 6 篇残留 **10 片 <50 字**；另有 08-21 e2e/dev 测试文档快照为 v1 时代产物（p50=17 字，重挖即刷新，非代码问题）。
- **形态定位**：残留碎片 100% 是**单行样板章节**——整节 = 标题 + 一句话（"应用限制
本特性无应用限制。"12–41 字），特性说明类文档成串出现。`_enforce_min` 的同章节合并够不到它们（前后均异章节），部分还夹在两个表格原子之间。**低于 min 但 ≥48 token 的短节（如 79 字"测量指标"）有独立检索价值，不应并入**。
- **决策（segment-compiler@2 → @2.1，跨章节合并的唯一例外）**：新增 `_absorb_micro_sections` pass——不足一行的微型节（< `_MICRO_ABSORB_TOKENS=48`）按文档顺序并入相邻文本片：优先并前邻（宿主身份不变，微型节标题已在其正文中不丢检索信息）；前邻是原子片时并后邻作前缀（后邻为表格即"文本前缀+表格"形态，复用引导句前缀机制，表格身份不变）；连续微型节先互相累积；越 max_tokens 或无宿主时保序保留。表格/图片原子规则不变（用户拍板"表格默认整表"——35 字单行发布历史表保持独立是**正确行为**）。`merge_adjacent_paragraphs=False` 时与 min 治理同门关闭。
- **顺手修复（存量顺序 bug）**：`_enforce_min` 表格前缀合并成功路径先 append 表格后 flush 更早的异章节小片，导致小片被甩到表格后、**文档顺序被打乱**（实盘：概述.md 的 License 段落在切片流中排到了"与其他特性的交互"表格之后）。改为先保序 flush 再挂表格。
- **真实 IR 离线重放（var/analysis/_replay_v21.py，MinIO 取 IR 本地重切）**：6 篇 50 片 → **41 片**，<50 字碎片 **10 → 1**（唯一残留 = 发布历史单行表原子，符合规则）。概述 16→10、参考信息 6→4、调测 6→5。
- **验证**：test_compiler_v2.py 新增 5 用例（样板节成串并入/表间微型节并后表前缀/文档头微型节并后邻/高于吸收线保持独立/越限保序）；segment_compiler 套件 35 passed；全量 1244 passed / 0 failed（含 DB 门控批次）。重刷方式不变：强制重挖（指纹变化→复用 IR 重切，不重新解析）。
