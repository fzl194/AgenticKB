...
NO_RESULT_LIMIT = 10
TOP_QUERY_LIMIT = 20
DASHBOARD_NO_RESULT_LIMIT = 5


async def _timed(
    view: str, segment: str, operation: Callable[[], Awaitable[T]],
) -> T:
    ...
    try:
        ...
        logger.info(
            "ops_usage_segment view=%s segment=%s duration_ms=%.1f",
            view, segment, (perf_counter() - started) * 1000,
        )
...
async def usage_stats(
    ...
    view: Literal["full", "dashboard"] = Query("full"),
    ...
    try:
        ...
        available = await _timed(view, "availability", stats.is_available)
        if not available:
        ...
        payload = {
            ...
            "days": days,
            "trend_days": TREND_DAYS,
            "summary": await _timed(
                view, "summary", lambda: stats.summary(domain=resolved, days=days)),
            "no_result_queries": await _timed(
                view, "no_result_queries",
                lambda: stats.no_result_queries(
                    domain=resolved, days=days,
                    limit=(DASHBOARD_NO_RESULT_LIMIT
                           if view == "dashboard" else NO_RESULT_LIMIT),
                ),
            ),
            "top_queries": [],
            "paradigms": await _timed(
                view, "paradigms",
                lambda: stats.paradigm_usage(domain=resolved, days=days)),
            "trend": await _timed(
                view, "trend",
                lambda: stats.trend(domain=resolved, days=TREND_DAYS)),
            "intents": {},
            "channels": {},
        }
        if view == "full":
            payload["top_queries"] = await _timed(
                view, "top_queries",
                lambda: stats.top_queries(
                    domain=resolved, days=days, limit=TOP_QUERY_LIMIT),
            )
            payload["intents"] = await _timed(
                view, "intents",
                lambda: stats.breakdown(
                    domain=resolved, days=days, column="intent"),
            )
            payload["channels"] = await _timed(
                view, "channels",
                lambda: stats.breakdown(
                    domain=resolved, days=days, column="channel"),
            )
        return payload
    finally:
        logger.info(
            "ops_usage_total view=%s duration_ms=%.1f",
            view, (perf_counter() - total_started) * 1000,
        )
